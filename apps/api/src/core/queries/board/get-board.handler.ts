...
export class GetBoardHandler implements IQueryHandler<GetBoardQuery, PiBoardProjection> {
  constructor(
  ...
  async execute(query: GetBoardQuery): Promise<PiBoardProjection> {
    const pi = await this.piRepo.findById(query.piId);
    if (!pi) throw new NotFoundException(`PI ${query.piId} not found`);
    ...
    for (const sprint of sprints) {
      ...
      if (load > sprint.capacity) {
        warnings.push({ type: 'OVERCOMMIT', sprintId: sprint.id, sprintName: sprint.name, currentLoad: load, capacity: sprint.capacity, overcommitPercent: Math.round(((load - sprint.capacity) / sprint.capacity) * 100) });
      }
    }
    ...
    return {
      ...
      sprints: sprints.map(s => ({
        id: s.id, piId: s.piId, name: s.name, order: s.order, capacity: s.capacity, currentLoad: sprintLoad.get(s.id) ?? 0,
        startDate: s.startDate ? s.startDate.toISOString().split('T')[0] : null, endDate: s.endDate ? s.endDate.toISOString().split('T')[0] : null,
